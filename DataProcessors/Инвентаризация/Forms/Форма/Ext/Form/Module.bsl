...
Процедура НоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ТекНом <> Неопределено Тогда
		...
		ИНаче
			КонтрольОткл();
			ЭтаФорма.Количество      = ПолучитьТхтОстатки(ТекНом,КонстантаСклад,КонтрольОткл,Истина,,Забаланс);
...
Процедура ОбновитьЗаголовокСОстатками()
	...
	ТекКол = 0;
	ЭтаФорма.Заголовок = "Остаток: "+ПолучитьТхтОстатки(ЭтаФорма.инвНоменклатура,КонстантаСклад,КонтрольОткл,ложь,ТекКол,Забаланс);
...
&НаСервереБезКонтекста
Функция ПолучитьТхтОстатки(текИнв,ТекСклад,КонтрольОткл,ВернутьЧисло=Ложь,фиксФакт=0,Забаланс=Ложь)
	...
	Запрос.УстановитьПараметр("Забаланс",Забаланс);
...
Процедура ОбновитьМеста(Команда=Неопределено,ТекущаяСтраница=Неопределено)
	...
	Если Элементы.ГрСтраницы.ТекущаяСтраница = Элементы.стМеста Тогда 
		...
		тблМеста.Очистить();
		Мас = ДанныеОстатки(КонстантаСклад,Место,Неопределено,Забаланс);
	...
	ИначеЕсли Элементы.ГрСтраницы.ТекущаяСтраница = Элементы.стНомен Тогда 
		...
		тблНом.Очистить();
		Мас = ДанныеОстатки(КонстантаСклад,Неопределено,текНом,Забаланс);
...
&НаСервереБезКонтекста
Функция ДанныеОстатки(ТекСклад,текМесто,текНом,Забаланс)
