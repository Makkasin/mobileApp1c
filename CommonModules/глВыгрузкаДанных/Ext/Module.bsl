...
Процедура ВыгрузитьЗаписиИнвентаризации(дтВыгр=Неопределено)  экспорт
	...
	Запрос.УстановитьПараметр("ИмяУстройства",глДоступ.ПолучитьНаименованиеУстройства());
...
&НаСервере
Процедура ВыгрузитьЗаписиИнвентаризацииОС()  экспорт
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |ГДЕ
	                |	инвИнвентаризацияОС.ДатаСинхронизации = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
					|  ";
	
	Запрос.УстановитьПараметр("ИдУстройства", Константы.ИдУстройства.Получить());
	Запрос.УстановитьПараметр("ИмяУстройства",глДоступ.ПолучитьНаименованиеУстройства());
	Запрос.УстановитьПараметр("ВидСпр","инвИнвентаризацияОС");
	...
	КонецЦикла;
	//тело = ЗаписатьХранилищеВСтроку(Тбл);
	
		 
		 тело = xmlСтрока(Новый ХранилищеЗначения(Тбл,Новый СжатиеДанных(5)));
		 ...
		 КонецеСЛИ;
			 ...
			 // защищенное соединение, если используется https
				 ...
				 Для каждого Стр  из Тбл Цикл
					 ...
					 Если Обк.ДатаИзменения <> Стр.ДатаИзменения Тогда Продолжить; КонецеслИ; // Пока записывали  данные поменялись
					 //ПрочитатьОтвет(Обк,Результат.ПолучитьТелоКакСтроку());
					 Обк.ДатаСинхронизации = ТекущаяДата();
...
&НаСервере
Процедура ВыгрузитьВыдачаТМЦ()  экспорт
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ЛОЖЬ КАК ЭтоГруппа,
	                |	ВерсияДанных КАК ВД,
	                |	ВыдачаТМЦ.ПометкаУдаления КАК ПометкаУдаления,
	                |	ВыдачаТМЦ.Ссылка КАК Ссылка1,
	                |	ВыдачаТМЦ.Дата КАК Дата,
	                |	ВыдачаТМЦ.Склад КАК Склад1,
	                |	ВыдачаТМЦ.Лог КАК Лог,
	                |	ВыдачаТМЦ.ГарНомер КАК ГарНомер,
	                |	ВыдачаТМЦ.ТабНомер КАК ТабНомер,
	                |	ВыдачаТМЦ.Сотрудник КАК Сотрудник,
	                |	ВыдачаТМЦ.ДокументОснование КАК ДокументОснование,
	                |	0 КАК ВидДвижения,
	                |	ВыдачаТМЦ.ИННДО КАК ИННДО,
	                |	&ИмяУстройства КАК ИмяУстройства,
	                |	&ИдУстройства  КАК ИдУстройства
					|
	                |ИЗ
	                |	Документ.ВыдачаТМЦ КАК ВыдачаТМЦ
	                |ГДЕ
	                |	ВыдачаТМЦ.ДатаСинхронизации = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)";
	...
	Тбл.Колонки.Добавить("GUID");
	Тбл.Колонки.Добавить("СкладGUID");
	Тбл.Колонки.Добавить("МатериалыTBL");
	Тбл.Колонки.Добавить("ФотоBIN");
	Для каждого Стр из Тбл Цикл
		Стр.GUID    		  = Стр.ссылка1.УникальныйИдентификатор();
		Стр.СкладGUID        = Стр.Склад1.УникальныйИдентификатор();
	Конеццикла;
		 ...
		 Для каждого стСкл из тбСкл Цикл
			 ...
			 Для каждого Стр из Тбл.НайтиСтроки(Новый Структура("Склад1",стСкл.Склад1)) Цикл
				 
				 пТ = Стр.Ссылка1.Материалы.Выгрузить();
				 пТ.Колонки.Добавить("НоменклатураGUID");
				 Для каждого пС из пТ Цикл
					пС.НоменклатураGUID = СокрЛП(пС.Номенклатура.УникальныйИдентификатор());	 
				 КонецЦикла;
				 пТ.Колонки.Удалить("Номенклатура");
				 
				 Стр.МатериалыTbl = пТ.Скопировать();
				 Стр.ФотоBIN = Стр.Ссылка1.фото.Получить();
				 пТ=Неопределено;
				 пМас = Новый Массив;
				 пМас.Добавить(Стр);
				 т1 = Тбл.Скопировать(пМас);
				 ...
				 Обк = Стр.ссылка1.ПолучитьОбъект();   
				 Если Обк.ВерсияДанных <> стр.ВД Тогда Продолжить; КонецЕСли; //пока выгружали уже кто-то изменил
				 Попытка
					 Обк.заблокировать();
				 Исключение
					 продолжить;
...
Процедура Обмен(Инд=0) Экспорт
		...
		КонецЕСЛИ;
		ВыгрузитьЗаписиИнвентаризацииОС();
...
Процедура ОтправитьНастройкуПланшетаНаСервере(ЭтоПовтор=Ложь) Экспорт
	...
	Пока Выб.Следующий() Цикл
		...
		Если выб.зн = Неопределено Тогда Продолжить; КонецЕСЛИ;
	...
	Соо.Вставить("ИдентификаторКлиента",СИ.ИдентификаторКлиента);
	Соо.Вставить("ВерсияПриложения",СИ.ВерсияПриложения);
