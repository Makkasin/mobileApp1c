...
Процедура ОбновитьМеста(Команда=Неопределено,ТекущаяСтраница=Неопределено)
    ...
    Если Элементы.ГрСтраницы.ТекущаяСтраница <> Элементы.стМеста Тогда Возврат; КонецЕСЛИ;
	
	тблМеста.Очистить();
	Мас = ДанныеОстатки(КонстантаСклад,Место);
...
&НаСервереБезКонтекста
Функция ДанныеОстатки(ТекСклад,текМесто)
	...
	Для каждого Стр из Тбл Цикл
		Стк = Новый Структура("Номенклатура,факт,учет,Другой");
